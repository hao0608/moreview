...
class HeartModelTest(TestCase):
    def setUp(self):
        ...
        self.heart=Heart.objects.create(
            user=self.user,
            review=self.review
        )
...
class ReviewCreateTest(TestCase):
    ...
    def test_review_create_can_work(self):
        ...
        self.assertRedirects(
            response, 
            expected_url=reverse("movie:detail", kwargs={"pk": self.movie.pk})
        )
...
class ReviewDeleteTest(TestCase):
    ...
    def test_review_delete_can_work(self):
        response=self.client.post(
            reverse("review:delete",kwargs={'pk':self.review.pk}),
            self.form
        )
...
class ReviewEditTest(TestCase):
    ...
    def test_revie_edit_can_work(self):
        response = self.client.post(
            reverse("review:edit",kwargs={"pk":self.review.pk}),
            self.form
        )
...
class HeartCreateTest(TestCase):
    ...
    def test_heart_create_can_work(self):
        ...
        self.form['heart']=""
        response = self.client.post(
            reverse("review:heart",kwargs={'pk':self.review.pk}),
            self.form
        )

        self.assertEqual(1,Heart.objects.filter(user=self.user.id,review=self.review.id).count())
        self.assertRedirects(
            response, 
            expected_url=reverse("movie:detail", kwargs={"pk": self.movie.pk})
        )
    ...
    def test_heart_delete_can_work(self):
        ...
        self.assertEqual(0,Heart.objects.filter(user=self.user.id,review=self.review.id).count())
        self.assertRedirects(
            response, 
            expected_url=reverse("movie:detail", kwargs={"pk": self.movie.pk})
        )